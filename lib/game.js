...
Game.prototype.play = function() {
    ...
    var currentHead = {x: this.snake.bodyArray[0].x,
                       y: this.snake.bodyArray[0].y
                      };

    var newPosition = this.setDirection(currentHead);

    this.checkIfGameOver(newPosition);

    var newCell = this.getCell(newPosition);

    this.snake.bodyArray.unshift(newCell);
    this.snake.fillSnake(this.board);

    this.timeOut = setTimeout(function(){game.play();}, this.speed);
};

Game.prototype.setDirection = function(position){
